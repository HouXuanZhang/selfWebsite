...
const gridImagesReady = preloadImages('.grid__item-img');
const startupDelay = new Promise(resolve => setTimeout(resolve, 3500));
...
// Large local photos should not block the first scroll experience indefinitely.
Promise.race([gridImagesReady, startupDelay]).then(startAnimations);
gridImagesReady.then(() => {
  if (animationsStarted && window.ScrollTrigger) {
